...
mixin CLTypedef!(cl_errcode function(cl_uint num_entries,
								  cl_platform_id* platforms,
...
version(CL_EXT)
{
	version(CL_VERSION_1_1)
	{
		...
		mixin CLTypedef!( cl_errcode function(cl_device_id device), "clReleaseDeviceEXT_fn");
	
		cl_errcode clRetainDeviceEXT( cl_device_id device); 
	
		mixin CLTypedef!( cl_errcode function(cl_device_id device), "clRetainDeviceEXT_fn");
	
		mixin CLTypedef!(cl_ulong, "cl_device_partition_property_ext");
		...
		mixin CLTypedef!( cl_errcode function(cl_device_id in_device,
											   const(cl_device_partition_property_ext)* properties,
											   cl_uint num_entries,
											   cl_device_id* out_devices,
